 ...
 * in painlessMesh including input validation, rate limiting, and performance
 * metrics.
 */
...
void setup() {
  ...
  Serial.println(
      "Features: Validation, Rate Limiting, Metrics, Memory Optimization");
}
...
void sendSensorData() {
  ...
  if (sent) {
    ...
    Serial.printf(
        "Sensor data sent: T=%.1f°C, H=%.1f%%, P=%.1f hPa, Batt=%d%%\n",
...
void handleCommand(alteriom::CommandPackage command, uint32_t from) {
  ...
  if (command.command == 1 && command.targetDevice == mesh.getNodeId()) {
    digitalWrite(LED_BUILTIN,
                 command.parameters.indexOf("ON") >= 0 ? HIGH : LOW);
...
void handleStatus(alteriom::StatusPackage status, uint32_t from) {
  Serial.printf("Status from %u: status=%d, uptime=%u, mem=%u\n", from,
                status.deviceStatus, status.uptime, status.freeMemory);
}
...
void newConnectionCallback(uint32_t nodeId) {
  ...
  metrics.topology_stats().record_topology_change(
      ...
      0  // Max hops calculation not implemented in this example
  );
}
...
void changedConnectionCallback() {
  ...
  auto nodeList = mesh.getNodeList();
  metrics.topology_stats().record_topology_change(
      nodeList.size() + 1, mesh.connectionCount(),
      0  // Max hops calculation not implemented in this example
  );
}
