...
void sendSensorData() {
  ...
  sensorData.temperature = 23.5 + random(-50, 50) / 10.0; // Simulate temperature
    ...
    case 200: // SensorPackage
      {
        ...
        Serial.printf("Sensor data from %u: T=%.1f, H=%.1f\n", 
                     receivedSensor.from, receivedSensor.temperature, receivedSensor.humidity);
    ...
    case 202: // StatusPackage
      {
        ...
        Serial.printf("Status from %u: uptime=%ds, mem=%dKB, fw=%s\n", 
                     ...
                     receivedStatus.freeMemory, receivedStatus.firmwareVersion.c_str());
      }
      break;
