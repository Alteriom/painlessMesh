...
class StationScan {
 public:
  ...
  uint16_t getConsecutiveEmptyScans() const { return consecutiveEmptyScans; }

  // Add a node to the TCP failure blocklist
  // This prevents repeated connection attempts to nodes where TCP server is
  // unresponsive
  void blockNodeAfterTCPFailure(uint32_t nodeId,
                                uint32_t blockDurationMs = 60000);
 ...
 protected:
  ...
  static const uint16_t EMPTY_SCAN_THRESHOLD =
      6;  // ~30 seconds at default SCAN_INTERVAL
  ...
  // Using 2^30 (~12 days) as reasonable limit - any time difference larger is
  // likely rollover
