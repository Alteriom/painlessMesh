...
public:
    ...
    bool allow_message(uint32_t node_id) {
        ...
        while (!history.empty() && (current_time - history.front()) > window_size_) {
            history.pop_front();
        }
    ...
    void clear_node_history(uint32_t node_id) {
        node_history_.erase(node_id);
    }
...
public:
     ...
     */
    ValidationResult validate_message(const JsonObject& obj, size_t message_size = 0) const {
        // Check message size
    ...
    const char* get_error_message(ValidationResult result) const {
        switch (result) {
            ...
            case ValidationResult::INVALID_JSON: return "Invalid JSON format";
            case ValidationResult::MISSING_REQUIRED_FIELD: return "Missing required field";
            case ValidationResult::INVALID_FIELD_TYPE: return "Invalid field type";
            case ValidationResult::INVALID_FIELD_VALUE: return "Invalid field value";
            case ValidationResult::MESSAGE_TOO_LARGE: return "Message too large";
            case ValidationResult::INVALID_NODE_ID: return "Invalid node ID";
            case ValidationResult::RATE_LIMIT_EXCEEDED: return "Rate limit exceeded";
...
public:
    ...
    static uint32_t generate() {
        ...
        if (!seeded) {
            ...
            uint32_t seed = std::chrono::duration_cast<std::chrono::microseconds>(duration).count();
            srand(seed ^ millis());
            seeded = true;
        }
